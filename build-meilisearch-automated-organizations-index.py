"""Build Meilisearch index from organization NDJSON files."""
...
def process_ndjson_files(index, organization_dir: Path) -> int:
    """Process ndjson files and add documents to Meilisearch index."""
    print("📦 Processing organization files...")

    ndjson_files = load_ndjson_files(organization_dir)
...
def main() -> None:
    """Main function to build Meilisearch index from organization files."""
    print("🚀 Starting Meilisearch organization index build...")

    print(f"Meilisearch URL: {SEARCH_API_URL}")

    # Locate data directory (database/organizations from generate-organizations.py)
    home_dir = Path.home()
    ...
    if not organizations_dir.exists():
        raise FileNotFoundError(
            ...
            f"Please run generate-organizations.py first."
        )
    ...
    try:
        ...
        document_count = process_ndjson_files(index, organizations_dir)

        print("\n✅ Meilisearch organization index build completed successfully!")
        print("📊 Summary:")
