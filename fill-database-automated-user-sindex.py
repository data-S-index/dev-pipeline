"""Compute AutomatedUserSIndex from AutomatedUserDataset and DIndex.
...
For each AutomatedUser, looks up datasets via AutomatedUserDataset, aggregates
DIndex scores by year for those datasets in Python. Processes in small batches.
"""
...
# Process this many automated users per batch (keeps memory and query size small)
...
def fetch_links_for_users(
    cur, user_ids: List[int]
) -> List[Tuple[int, int]]:
    ...
    return [(r["automatedUserId"], r["datasetId"]) for r in cur.fetchall()]
...
def aggregate_sindex_in_python(
    ...
    """
    Aggregate (automatedUserId, year) -> sum(score) in Python.
    Returns list of (automatedUserId, score, year) for insert.
    """
    # datasetId -> [(year, score), ...]
    ...
    # (userId, year) -> total score
    user_year_score: dict[Tuple[int, int], float] = defaultdict(float)
    for user_id, dataset_id in links:
        for year, score in by_dataset.get(dataset_id, []):
            user_year_score[(user_id, year)] += score

    return [
        (user_id, score, year)
        for (user_id, year), score in user_year_score.items()
    ]
...
def compute_and_write_sindex_ndjson(conn: psycopg.Connection, output_dir: Path) -> int:
    """
    Process automated users in batches: load links and DIndex, aggregate in Python,
    write (automatedUserId, score, year) to local NDJSON files. Returns total rows written.
    """
...
def load_sindex_from_ndjson(conn: psycopg.Connection, input_dir: Path) -> int:
    """Load AutomatedUserSIndex from local NDJSON files into the database. Returns rows loaded."""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute AutomatedUserSIndex (write NDJSON) or load from NDJSON into DB."
    )
    parser.add_argument(
        "--load",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory for NDJSON (default: ~/Downloads/database/automatedusersindex)",
    )
    args = parser.parse_args()

    downloads_dir = Path.home() / "Downloads"
    database_dir = downloads_dir / "database"
    default_output = database_dir / "automatedusersindex"
    ...
    if args.load:
        ...
        print("🚀 Loading automated user s-index from NDJSON...")
        try:
