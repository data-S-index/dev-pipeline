...
def process_fuji_files(conn: psycopg.Connection, fuji_dir: Path) -> int:
    """Stream Fuji NDJSON files and upsert into FujiScore table in a single pass."""
    ...
    pbar = tqdm(desc="  Processing", unit="record", unit_scale=True)
...
def main() -> None:
    ...
    fuji_dir = downloads_dir / "pulled-database" / "fuji-scores"
    ...
    if not fuji_dir.exists():
        raise FileNotFoundError(
            ...
            "Please run fuji-score-extrapolation-update.py first."
        )
