"""Generate a distinct list of unique authors from pull-dataset-authors.py output (dataset NDJSON)."""
...
def author_canonical_key(author: Dict[str, Any]) -> tuple:
    ...
    return ("by_name_affiliation", name, affiliations_key)
...
def main() -> None:
    """Read pull-dataset-authors.py output, collect unique authors, write NDJSON batches."""
    print("🚀 Generating unique authors from dataset NDJSON...")

    dataset_dir = Path("E:/s-index-custom") / "dataset"
    output_dir = Path.home() / "Downloads" / "pulled-database" / "automated-author"
    authors_dir = output_dir / "authors"
    automateduserdataset_dir = output_dir / "automateduserdataset"

    if not dataset_dir.exists():
        raise FileNotFoundError(
            f"Dataset directory not found: {dataset_dir}. "
            "Run pull-dataset-authors.py first to generate dataset NDJSON files."
        )
    ...
    for out_dir in (authors_dir, automateduserdataset_dir):
        if out_dir.exists():
