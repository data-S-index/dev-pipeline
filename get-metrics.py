...
- datasetsByYear: counts by year (1950-2010, 2011..2026)
...
YEAR_BUCKETS = ["1950-2010"] + [str(y) for y in range(2011, 2027)]
...
def datasets_by_year(conn: psycopg.Connection) -> list[dict]:
    ...
    SELECT
        CASE
            WHEN d."pubYear" < 2011 THEN '1950-2010'
            ELSE d."pubYear"::text
        END AS year,
        COUNT(*) AS value
    FROM "Dataset" d
    WHERE d."pubYear" BETWEEN 1950 AND 2026
    GROUP BY 1
    ORDER BY MIN(d."pubYear");
    """
