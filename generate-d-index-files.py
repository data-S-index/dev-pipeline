...
Input: NDJSON produced by pull-db-for-d-index.py, under
  ~/Downloads/pulled-database/datasets/*.ndjson. Each record carries everything needed to
  recompute the d-index: id, publishedAt, fuji.score, citations[], mentions[], datasetTopic.topicId.
  This script never connects to the database directly - run pull-db-for-d-index.py first.

Normalization: Subfield+year via subfield_norm_factors.duckdb and
  openalex_topic_mapping_table.csv in this repo's root (required; no default).
...
def _fast_count_lines(file_path: Path) -> int:
    """Count newlines by scanning raw bytes (used only to size the progress bar)."""
    total = 0
...
def _parse_input_record(
    ...
    """Convert one pull-db-for-d-index.py NDJSON record into the
    (dataset_id, published_at, topic_id, fair_score, citations, mentions) shape."""
    dataset_id = int(record["id"])
    published_at = _to_datetime_utc(record.get("publishedAt"))
    topic_id = (record.get("datasetTopic") or {}).get("topicId")
    fuji = record.get("fuji") or {}
    fair_score = fuji.get("score")
    fair_score = float(fair_score) if fair_score is not None else 0.0
    citations = [
        (
            _to_datetime_utc(c.get("citedDate")),
            float(c.get("citationWeight", 1.0) or 1.0),
        )
        for c in (record.get("citations") or [])
    ]
    mentions = [
        {
            "mention_date": m.get("mentionedDate"),
            "mention_weight": float(m.get("mentionWeight", 1.0) or 1.0),
        }
        for m in (record.get("mentions") or [])
    ]
    return dataset_id, published_at, topic_id, fair_score, citations, mentions
...
def main() -> None:
    ...
    output_dir = downloads_dir / "pulled-database" / "dindex"
    norm_dir = downloads_dir / "pulled-database" / "normalization"

    if not input_dir.exists():
        raise FileNotFoundError(
            f"Input directory not found: {input_dir}. "
            "Run pull-db-for-d-index.py first to generate dataset NDJSON files."
        )

    # Subfield normalization is required; it lives alongside this script.
    ...
    # Locate input NDJSON files (pull-db-for-d-index.py output)
    ndjson_files = load_ndjson_files(input_dir)
    if not ndjson_files:
        print(f"  ⚠️  No input NDJSON files found in {input_dir}; nothing to do.")
        return
    print(f"  Found {len(ndjson_files)} input file(s)")

    total_datasets = _count_records(ndjson_files)
    print(f"\n📈 Calculating d-index values for {total_datasets:,} datasets...")
    if N_WORKERS > 0:
        print(f"  Using {N_WORKERS} worker process(es) for parallel computation.")
    ...
    def flush_rows_buffer() -> None:
        nonlocal rows_buffer, current_batch, file_number, total_records, processed_datasets
        if not rows_buffer:
            return

        # Build list of records for this buffer (parallel or sequential)
        batch_records: List[dict] = []
        if N_WORKERS > 0:
            chunk_size = max(1, len(rows_buffer) // N_WORKERS)
            chunk_payloads = []
            for i in range(0, len(rows_buffer), chunk_size):
                ...
                chunk_payloads.append(
                    (
                        ...
                        use_subfield_norm,
                        str(topics_table_path) if topics_table_path else None,
                        topic_to_subfield,
                        subfield_norm_cache,
                    )
                )
        ...
        processed_datasets += len(rows_buffer)
        ...
        for record in batch_records:
            ...
            if len(current_batch) >= BATCH_SIZE:
                ...
                write_normalization_batch_to_file(current_batch, file_number, norm_dir)
                file_number += 1
                current_batch = []
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    try:
                        ...
                        continue
                    rows_buffer.append(_parse_input_record(record))
